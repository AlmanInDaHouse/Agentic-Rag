...
export function renderMarkdownReport(report: RetrievalEvalReport): string {
  ...
  for (const result of report.results) {
    lines.push(
      ...
      `Effective minRequiredScore: ${format(result.answerability.effectiveMinRequiredScore ?? result.answerability.minRequiredScore)}`,
      `Effective fallbackAllowed: ${String(result.answerability.effectiveFallbackAllowed ?? true)}`,
      `Effective policy source: ${result.answerability.effectivePolicySource?.join(", ") ?? "unknown"}`,
