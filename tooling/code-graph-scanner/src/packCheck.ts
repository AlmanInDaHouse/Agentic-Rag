...
const defaultExpectedPath = "expected/code-context-pack.normalized.json";
const defaultSourceArtifactPath = "artifacts/code-graph/code-graph.json";

async function main(): Promise<void> {
  const options = await parsePackCheckArgs(process.argv.slice(2));
  ...
  const normalized = normalizeContextPack(contextPack);
  ...
  if (actualJson !== expectedJson) {
    throw new Error(`Code Graph context pack fixture output drifted from ${path.relative(workspaceRoot, options.expectedPath).replaceAll("\\", "/")}`);
  }

  process.stdout.write(`Code Graph context pack fixture check passed for ${path.relative(workspaceRoot, options.fixtureRoot).replaceAll("\\", "/")}\n`);
}

export async function parsePackCheckArgs(args: string[]): Promise<PackCheckOptions> {
...
async function resolveInsideWorkspace(input: string): Promise<string> {
  ...
  if (!isInside(realWorkspaceRoot, realResolved)) {
    throw new Error(`Code Graph context pack check path escapes the repository: ${input}`);
  }
  return realResolved;
}
