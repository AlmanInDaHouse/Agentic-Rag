...
describe("harness: context duplicate document policy", () => {
  ...
  it("returns 409 when the same source receives the same normalized content twice", async () => {
    ...
    const input = {
      ...
      content: "Duplicate content is rejected by stable hash.",
      metadata: {}
    };

    await runtime.api.addContextDocument(source.id, input);

    expect(await runtime.api.addContextDocumentStatus(source.id, input)).toBe(409);
  });
});
