...
- `apps/api`: Fastify ESM API, PostgreSQL repositories, migrations, mock debate runtime, mock agent runtime state machine, safe execution policy, Context Engine, basic context redaction/retention policy, mock embedding boundary, optional local/pgvector capability reporting, optional pgvector active retrieval, the provider framework under `src/providers/` (A2 mock adapters + black-box conformance harness + quota manager; A3 real read-only Codex/Claude adapters in `src/providers/real` over an injectable `ProcessRunner`, NOT wired into the runtime), the A4 collaboration runtime under `src/orchestration/` (Specialist/Pair/Full Debate modes, mode selection, review protocol, authority-order strategy resolution and quota-gated provider steps — pure, deterministic, mock-first, no real writes, NOT wired into the runtime), and tests.
- `apps/web`: React + Vite dashboard.
- `packages/shared`: Zod contracts and shared TypeScript types, including the A1 provider contracts under `src/provider/` (provider-agnostic adapter interface, 13-event `ProviderEvent` union, capability snapshots and 12 artifact contracts).
...
The MVP supports goal creation/listing, debate round creation, latest round retrieval, mock agents, mock judge, persisted proposals, timeline events and basic dashboard. Milestone 1.3.1 adds a persisted mock agent runtime state machine with approval gate workflows, a safe execution policy, transactional advance locking and approval hardening. Milestone 1.4 adds Context Engine v0 with manual/project/artifact sources, deterministic chunking, lexical retrieval and runtime `load_context` integration. Milestone 1.5C-A adds deterministic regex redaction and context data policy metadata before future real embedding work. Milestone 1.5C-B adds basic context retention quotas, soft delete/restore and audit events. Milestone 1.5D adds optional pgvector active retrieval when explicitly configured and available, with JSONB/mock/lexical fallback when it is not. Milestone 1.5E adds deterministic retrieval evaluation fixtures, metrics and report generation. Milestone 1.5F adds versioned retrieval baselines and quality gates for the synthetic evaluation harness. Milestone 1.5G expands the synthetic retrieval evaluation corpus with query types, tags, ambiguous retrieval, redaction and no-answer cases. Milestone 1.5H adds deterministic RAG abstention policy based on retrieval metadata. Milestone 1.5I calibrates abstention thresholds by mode, query type and fallback use. Milestone A1 (the first milestone of the provider reorientation) adds provider-agnostic provider contracts in `packages/shared` (`ProviderAdapter` interface, `ProviderEvent` 13-event discriminated union with terminal semantics, `CapabilitySnapshot`, and the 12 Zod artifact contracts incl. `TaskProfile`/`RoutingDecision`/`GovernanceDecision`), with schema versioning/compatibility rules, no provider-specific logic, and pure schema tests; no adapters were implemented in A1. Milestone A2 adds the deterministic mock Codex/Claude adapters, a reusable black-box conformance harness and the quota manager (budgets, reserves, hard stops, unknown state). Milestone A3 adds the real read-only Codex/Claude adapters in `apps/api/src/providers/real`: an injectable `ProcessRunner` (production `NodeProcessRunner` is the only `child_process` site; tests use a `FakeProcessRunner` replaying synthetic fixtures of the documented CLI output), per-provider event normalizers, availability/version/non-secret-auth/version-bound-capability probes, and read-only headless execution with process-group cancellation/timeout — validated by the A2.2 harness unchanged and NOT wired into the runtime (event schemas remain versioned assumptions verified by a manual live smoke, not CI). Milestone A4 adds the collaboration runtime in `apps/api/src/orchestration`: a pure, deterministic, mock-first orchestration layer that coordinates planning, critique, resolution and review between two providers over the A1 contracts, driven by the A2 mock adapters and the quota manager. It implements the Specialist, Pair and Full Debate modes, risk/uncertainty mode selection with budget-aware downgrades (mirroring the reserves), capability-primary owner selection (`RoutingDecision`), the review protocol (`ReviewFindings` with the nine mandated fields + a severity gate), and authority-order strategy resolution (`StrategyDecision`, never agent majority). Every provider step is quota-gated (reserve → run → commit/release) and every stage is a Zod-validated A1 artifact; the "execution" step is simulated via the mock event stream (read-only). A4 performs no real writes and no real CLI execution (writable execution is A5, gated on A0.5) and is NOT wired into the routes. The runtime remains mock-only and does not execute real commands, modify code, install dependencies, run migrations, generate LLM answers or call real adapters.
...
- Retrieval evaluation uses deterministic synthetic fixtures, query metadata, simple metrics, abstention metrics, versioned thresholds and compact baselines; no-answer abstention accuracy can block, but no LLM-as-judge or real model is required.
...
- Provider contracts (A1, ADR 0033) live in `packages/shared` as provider-agnostic Zod schemas + TypeScript types; the only provider-specific value is the `"codex" | "claude"` enum. Contracts precede adapters; the event/capability schemas are versioned assumptions (`PROVIDER_CONTRACT_SCHEMA_VERSION`) verified per CLI version, with `unknown` preferred over fabrication.
- The A4 collaboration runtime (ADR 0035) is pure orchestration over A1/A2/A3, mock-first and read-only: it never writes or runs a real CLI (writable execution is A5, gated on A0.5). Conflict resolution follows an authority order (safety invariants → spec → acceptance criteria → code evidence → tests → ADRs → threat model → risk policy → governance decision), never agent majority or confidence; mode selection downgrades rather than overspending a budget; A4 produces no `GovernanceDecision` (that binds a writable capability and belongs to A5).
...
- RAG v1 has deterministic mock embeddings, optional local/pgvector capability reporting, optional pgvector active retrieval, hybrid/mock-vector modes, deterministic calibrated answerability/abstention metadata and a deterministic retrieval evaluation harness with expanded synthetic quality gates, but no required real semantic embeddings, external providers, LLM-as-judge, answer generation, generation faithfulness evaluation or production vector tuning yet.
