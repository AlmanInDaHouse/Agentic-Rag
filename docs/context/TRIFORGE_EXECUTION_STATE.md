...
**Last updated:** 2026-06-30 (Loop 52 — **A10-W.8b integrated-runtime CODE complete + A10-W.9 security review + gate hardening merged/in-flight**. W.8b shipped as 4 small PRs: #90 PowerShell lifecycle scripts (setup/dev/start/stop/status/clean), #91 integrated runtime core (`TRIFORGE_PROVIDER_MODE` mock|real, `createConfiguredAdapter` no-fallback, `IntegratedRunService` over the writable pipeline with a sequence-numbered event stream), #92 persistence + HTTP API (`integrated_runs`/`integrated_run_events`, `PgIntegratedRunStore`, `/api/integrated-runs/*`, server wiring), #94 product UI (`IntegratedRunConsole` with honest provenance + live timeline, mounted by `main.tsx`). W.9: adversarial 34-vector security review (0 blockers/criticals, 4 majors — 1 fixed: `NodeGitRunner` executable-shadowing → absolute git via `resolveGitExecutable`; 3 accepted OS-sandbox residuals RR-4/R-WIN-3..5), final-gate hardening test (pins the 19 mandatory real caps; no-false-green), clean-install install/build/doctor verified on a fresh NTFS checkout. **Remaining = the 3 real verifications, gated on ONE owner action (local DB) + real-provider quota.** CI stays Linux-green.)

## Snapshot

| Field | Value |
|---|---|
| Last closed milestone | A10-W.8b integrated-runtime CODE (PRs #90–#92, #94) + A10-W.9 security review / gate hardening / git-fix (PR in flight) |
| Active milestone | **A10-W — Native Windows Operational Closure** (ADR 0056). Substrate (W.1–W.5) `verified_real_environment`; adapters (W.6) / pilots (W.7) / modes+quota (W.8a) `verified_real_provider`. **W.8b runtime is now WIRED** (provider-mode mock/real, integrated API+persistence+UI). **Remaining: the 3 real verifications (W.8b E2E + restart recovery, W.9 clean-install full) + tag `v1.0.0`.** |
| `main` SHA | `aa997ed` (PR #94; the W.9 review/gate/git-fix PR is in flight) |
| Last `main` CI | `Validate` ✅ success |
| Open PRs | the A10-W.9 security-review/gate/git-fix PR (self-merge) |
| Evidence | **11 `verified_real_provider` + 5 `verified_real_environment` mandatory caps.** Still only **3 mandatory caps `unknown`**: `windows_integrated_product_e2e` (real provider), `windows_restart_recovery` (real env), `windows_clean_install` (real env). Clean-install is install/build/doctor-verified (see `docs/evidence/CLEAN_INSTALL_VERIFICATION.md`) but NOT flipped — setup/start need the DB. (`competitive_mode_real` non-mandatory.) |
| Blockers | **ONE owner action (external):** the local application DB. The `triforge` Postgres role fails SCRAM auth and the superuser password is owner-only; Docker daemon is stopped. The integrated server (hence E2E #5, restart recovery #4, full clean-install #7) cannot run without it. **Plus real-provider quota** for the one integrated real E2E (Claude 7-day window ~0.85; use Codex as owner per §20). No internal code blocker — the runtime is built and CI-green. |
| Pending decisions | none |
| Next loop (exact) | **Owner provisions the local DB** (one of, run via `! …`): native — `psql -U postgres -c "CREATE ROLE triforge LOGIN PASSWORD 'triforge' CREATEDB; CREATE DATABASE triforge OWNER triforge;"`; or start Docker Desktop (compose Postgres on :5433, set `DATABASE_URL`). **Then:** (1) `pnpm triforge:setup && pnpm triforge:start`, browser-drive a real integrated run on a DISPOSABLE fixture (Codex owner to conserve Claude) → flip `windows_integrated_product_e2e`=`verified_real_provider`; run the cancellation E2E. (2) Restart-recovery on the host (persist → restart backend → `recover()` → single terminal → no double-merge) → flip `windows_restart_recovery`=`verified_real_environment`. (3) Full clean-install incl. setup/start → flip `windows_clean_install`=`verified_real_environment`. (4) When `evaluateFinalReleaseReadiness` is green: update release notes RC→final, tag `v1.0.0`, GitHub Release. **`v1.0.0` is NOT tagged; DoD is NOT yet met** (3 caps `unknown`, gate red — correctly). |
